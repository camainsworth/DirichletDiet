...
#'  
#' See publication:
#' Ainsworth, C.H., Kaplan, I.C., Levin, P.S. and Mangel, M. (2010)  A statistical approach 
#' for estimating fish diet compositions from multiple data sources: Gulf of California case
#' study. Ecological Applications, 20(8): 2188-2202.
#' 
#' Contributing authors to date: Ainsworth, Chagaris, Masi, Tarnecki, Morzaria Luna, Scott
#' 
#' This code uses a maximum likelihood fitting procedure to fit a Dirichlet distribution to
#' multivariate predator-prey data.  It outputs the marginal beta distribution parameters,
#' the beta distributions themselves (useful for sampling), and graphs of the marginal beta 
#' distributions
#' 
#' 
#' 
#--------------------------------FUNCTIONS--------------------------------------
#this function checks if a package is installed, if not it installs the package.
SmartInstall <- function(WhatPackage){ 
  WhatRepository = c("http://packages.ropensci.org", "http://cran.rstudio.com")
  for (i in 1:length(WhatPackage)){
    if (WhatPackage[i] %in% rownames(installed.packages()) == FALSE){  
    install.packages(WhatPackage,repos = WhatRepository)          
    }
  }
}
...
.packages = c("XMLSchema","SSOAP","data.table","dplyr","ggplot2","stringi","tidyr","stringr",
              "R.utils","data.table","ggExtra", "ggthemes","Hmisc","VGAM",
              "future","doSNOW","magrittr")
SmartInstall(.packages)

# Load packages into session 
lapply(.packages, require, character.only=TRUE)

# clean up the space 
rm(list=ls()) #clears memory of all active functions and variables
graphics.off() #closes any graphs that are open
...
workpath <- "C:/Users/ainsworth/Documents/GitHub/DirichletDiet"
savepath <- "C:/Users/ainsworth/Documents/GitHub/DirichletDiet/Beta"
